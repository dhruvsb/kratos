...
lost) — see `FEEDBACK-LOG.md` #10. Also logged (not yet fixed): **#11** delete-set exists but is
undiscoverable · **#12** weight→reps auto-advance conflicts with 3-digit weights, resolve after
**#13** · **#13** reps as 5 fixed buttons needs an escape hatch. Prior: 2026-08-06 (evening) — **Showcase-finish pass:** app icon + splash are now the
...
- [ ] **Fix #11 (delete-set discoverability)** — swipe-to-delete on a logged set row is the
      state-of-the-art answer (`react-native-gesture-handler` already a dependency).
- [ ] **Decide + build #12/#13 together** — fixed rep-count chips (4/6/8/10/12 + an escape hatch
      for reps outside that set) likely removes the need for weight→reps auto-advance entirely.
